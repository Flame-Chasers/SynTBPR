...
def save_image(image: Image, output_folder: str, image_id: int, random_value: int, style_index: int):
    if not os.path.exists(output_folder):
        os.makedirs(output_folder)
    output_path = os.path.join(output_folder, f'images_{image_id}_{random_value}_{style_index}.jpg')
...
def generate_images_from_json(json_path: str, output_folder: str, config: Dict, start_index: int, num_entries: int):
    ...
    for entry in tqdm(data, desc="Generating images", unit="entry"):
        ...
        for idx, style in enumerate(config["styles"], 1):  
            ...
            save_image(outputs.images[1], output_folder, image_id, random_value, idx)
