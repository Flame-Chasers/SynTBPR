...
class TextTransformer(nn.Module):
    ...
    def forward(self, text=None, input_embeds=None, collect_ind=None, return_all_features: bool = False):
        if text is None and input_embeds is None:
            raise ValueError("Either text or in input_embeds should be provided")
        if text is not None and input_embeds is not None:
            raise ValueError("Only one of text or input_embeds should be provided")

        cast_dtype = self.transformer.get_cast_dtype()
        if text is not None:
            input_embeds = self.token_embedding(text).to(cast_dtype)  # [batch_size, n_ctx, d_model]
            collect_ind = text.argmax(dim=-1)

        x = input_embeds + self.positional_embedding.to(cast_dtype)
        x = x.permute(1, 0, 2)  # NLD -> LND
        ...
        if not return_all_features:
            ...
            x = x[torch.arange(x.shape[0]), collect_ind] @ self.text_projection
        return x
